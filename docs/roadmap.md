...
2. 按 `docs/plans/2026-04-19-cli-maintainability-refactor-plan.md` 继续推进 CLI 维护性重构；在当前这一轮收口后，下一步重点已不再是机械拆小，而是评估 `DoctorEnvironmentResolutionCommands`、`AudioGainExecutionCommands`、`BeatTrackExecutionCommands`、`TranscribeExecutionCommands`、`AudioAnalyzeExecutionCommands`、`SeparateAudioCommands`、`DetectSilenceCommands` 这些“单命令但仍偏大”的 execution partial 是否还值得继续细分，或是否已经达到“单命令 / 单主题即可停止”的边界。
