...
2. 按 `docs/plans/2026-04-19-cli-maintainability-refactor-plan.md` 继续推进 CLI 维护性重构；在已完成 `Program.cs` 命令族迁移与多轮测试 owner 收口后，下一步重点转为评估 `RenderPreviewCommands`、`TemplateGuideCommands`、`InitPlanTranscriptSeedCommands`、`AudioGainCommands` 这些仍偏大的 partial 是否还值得继续细分，或是否已经达到“单命令 / 单关注点即可停止”的边界。
