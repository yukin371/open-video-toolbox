...
- `audio-gain` 只负责解析增益参数、组织输出路径与返回执行结果；传 `--json-out` 时只负责把同一份结构化结果 envelope 落盘，不持有 `ffmpeg volume` 规则
...
- `separate-audio` 只负责分离参数解析、输出目录落盘和结构化摘要；传 `--json-out` 时只负责把同一份结构化结果 envelope 落盘，不持有 `demucs` 目录映射或命令拼接规则
