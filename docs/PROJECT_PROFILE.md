...
- `init-narrated-plan` 已实现，用于从显式 narrated manifest 装配讲解型 `schemaVersion = 2` `edit.json`；当前支持 `video + image` 两类 visual section，owner 仍固定在 `Core.Editing` + `Cli` glue，不纳入现有 `templates` catalog，也不引入 AI/TTS provider。
...
- 当前测试基线：`OpenVideoToolbox.Core.Tests` 166，`OpenVideoToolbox.Cli.Tests` 182，总计 348。
