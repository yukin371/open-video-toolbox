...
- 命令分发：`presets`、`templates`、`effects`、`doctor`、`validate-plugin`、`init-plan`、`init-narrated-plan`、`init-narrated-plan-batch`、`scaffold-template`、`scaffold-template-batch`、`beat-track`、`audio-analyze`、`audio-gain`、`audio-normalize`、`transcribe`、`detect-silence`、`auto-cut-silence`、`separate-audio`、`probe`、`plan`、`run`、`cut`、`concat`、`extract-audio`、`subtitle`、`validate-plan`、`inspect-plan`、`replace-plan-material`、`replace-plan-material-batch`、`attach-plan-material`、`attach-plan-material-batch`、`bind-voice-track`、`bind-voice-track-batch`、`export`、`mix-audio`、`render`、`render-batch`
- 参数解析、默认值选择、帮助输出
- CLI 级错误提示和退出码语义
- 稳定的结构化输出契约
- batch 结果目录、`summary.json` / `results/<id>.json` 与 batch artifact helper 的组织边界（当前 owner 文件：`BatchCommandArtifacts.cs`）
...
- 音频 / speech / subtitle / silence-driven draft 命令 wrapper 的组织边界（当前 owner 文件：`AudioCommandHandlers.cs`）
- `validate-plan` / `inspect-plan` / `replace-plan-material` / `attach-plan-material` / `bind-voice-track` 及其 batch 入口的组织边界（当前 owner 文件：`EditPlanCommandHandlers.cs`）
- `export` / `mix-audio` / `render` 执行命令 wrapper 与 plan loading glue 的组织边界（当前 owner 文件：`RenderCommandHandlers.cs`）
...
- narrated-slides manifest 装载、变量文件 overlay、相对路径解析、时长探测 glue 与 command envelope 输出的组织边界（当前 owner 文件：`NarratedSlidesPlanBuildSupport.cs`、`TemplateCommandHandlers.cs`）
...
- `init-narrated-plan` 只负责消费 narrated manifest、可选装载 `--vars <vars.json>` 变量文件、解析相对路径、在必要时通过 `ffprobe` 探测未显式声明的 section 时长，并把 narrated plan 生成与 `${var}` 解析规则留在 `Core.Editing`
- `init-narrated-plan` 对 narrated `bgm.slot` 只负责装载和路径判空；是否产出 `bgm` 轨、何时失败仍由 `Core.Editing` 持有
- `init-narrated-plan` 对 narrated `visual.slot` 只负责装载、路径判空与缺视觉时跳过文件探测；是否投影 placeholder、何时失败仍由 `Core.Editing` 持有
- `init-narrated-plan-batch` 只负责解析 batch manifest、把 `manifest/output/renderOutput/vars` 解析到 batch manifest 目录、逐项复用单项 narrated build 路径，并汇总 `results[]` / `summary.json` / `results/<id>.json` / 稳定退出码；单份 narrated manifest 的变量解析、slot 裁剪与 plan 投影语义仍由 `Core.Editing` 持有
...
- `scaffold-template-batch` 只负责解析 batch manifest、把相对路径解析到 manifest 目录、为每个条目复用单项 `scaffold-template` 执行路径，并汇总 `results[]` / `summary.json` / `results/<id>.json` / 稳定退出码；单任务 plan 生成、guide/example 写盘与 validation 语义仍由既有 owner 组合持有
...
- `EffectsCommandHandlers.cs` 负责 `effects` discovery 命令 wrapper；CLI 只透传 `Core.Editing` 的 effect 描述符，不维护第二套 effect schema
...
- `TemplateCommandHandlers.cs` 负责 `templates`、`init-plan`、`scaffold-template`、`scaffold-template-batch` 这组模板命令的入口 wrapper；`Program.cs` 只保留顶层分发，不再保留这组 wrapper 的内联实现
- `init-narrated-plan` 当前作为模板相关命令族中的独立显式入口保留在 `TemplateCommandHandlers.cs`；CLI 只透传 `Core.Editing` 的 narrated plan 生成结果，不把它偷偷并入现有 `templates` catalog
- `MediaCommandHandlers.cs` 负责 `cut`、`concat`、`extract-audio` 这组媒体基础命令的入口 wrapper；`Program.cs` 只保留顶层分发，不再保留这组 wrapper 的内联实现
- `AudioCommandHandlers.cs` 负责 `audio-analyze`、`audio-gain`、`audio-normalize`、`transcribe`、`detect-silence`、`auto-cut-silence`、`separate-audio`、`beat-track`、`subtitle` 这组音频 / speech / silence-driven draft 命令的入口 wrapper；`Program.cs` 只保留顶层分发，不再保留这组 wrapper 的内联实现
- `EditPlanCommandHandlers.cs` 负责 `validate-plan`、`inspect-plan`、`replace-plan-material`、`replace-plan-material-batch`、`attach-plan-material`、`attach-plan-material-batch`、`bind-voice-track`、`bind-voice-track-batch` 这组 edit-plan inspection / material mutation 命令的入口 wrapper；`Program.cs` 只保留顶层分发，不再保留这组 wrapper 的内联实现
...
- `auto-cut-silence` 只负责加载 `silence.json`、解析 CLI 阈值与模式、决定总时长来源顺序（`--source-duration-ms` 优先，否则探测 `silence.json.inputPath`）、调用 `Core.Editing.AutoCutSilencePlanner` 并输出统一 command envelope；它不拥有非静音区间算法，也不把结果自动接进 `render` / `run`
...
- `effects` 只负责 effect list / describe 的命令入口、过滤条件与 envelope 输出；effect 参数 schema、template mode 与 built-in catalog 仍由 `Core.Editing` 持有
- `inspect-plan`、`replace-plan-material`、`replace-plan-material-batch`、`attach-plan-material`、`attach-plan-material-batch`、`bind-voice-track` 负责加载 `edit.json`、解析 CLI 选择器与路径写回策略、调用 `Core.Editing` 的既有 plan 语义，并输出稳定结构化结果；它们不拥有第二套 plan 模型，也不承担通用 JSON patch
- `inspect-plan` 的 `signals[]` 总状态字段必须原样透传 `Core.Editing` 的稳定取值，不能在 CLI 层重写另一套“建议动作”语义
- `replace-plan-material-batch` 负责解析批量 manifest、把相对路径解析到 manifest 目录、逐项复用单项 `replace-plan-material` 流程，并汇总 `results[]` / `summary.json` / `results/<id>.json` / 稳定退出码；单份 plan 的 mutation / validation 语义仍由 `Core.Editing` 持有
- `attach-plan-material-batch` 负责解析批量 manifest、把相对路径解析到 manifest 目录、逐项复用单项 `attach-plan-material` 流程，并汇总 `results[]` / `summary.json` / `results/<id>.json` / 稳定退出码；单份 plan 的 mutation / validation 语义仍由 `Core.Editing` 持有
- `bind-voice-track-batch` 负责解析批量 manifest、把相对路径解析到 manifest 目录、复用单项 `bind-voice-track` 流程，并汇总 `results[]` / `summary.json` / `results/<id>.json` / 稳定退出码；单份 plan 的 mutation / validation 语义仍由 `Core.Editing` 持有
- `mix-audio` 只负责加载 `edit.json`、应用输出路径 override，并把音频执行委托给 `Core.Execution`
- `export` 只负责加载 `edit.json`、解析 `--format/--output/--frame-rate/--overwrite`、把相对输出路径解析到 plan 目录，并把导出委托给 `Core.Execution`；它不得持有 v1 包装、v2 主轨选择或 EDL 文本拼接规则
...
- `render-batch` 只负责解析批量 manifest、把相对路径解析到 manifest 目录、加载多份 `edit.json`、逐项复用单项 `render` 的 preview / execute 语义，并汇总 `results[]` / `summary.json` / `results/<id>.json` / 稳定退出码；它不拥有第二套 render 模型，也不重写 `Core.Execution` 的执行状态语义
- 外部 AI 只能消费 CLI 输出，不能要求仓库内出现新的 AI provider 层

## 常见坑

- 新增 CLI 选项时，容易把默认值、路径规则或命令拼接塞回入口层
- `effects` 的返回字段必须直接由 `Core.Editing` 描述符派生，不能在 CLI 里偷偷扩出另一套 effect 配置模型
...
- 对字幕 / signal 模板，`commands.json` / `commands.*` 还必须把 attach 后的 `inspect-plan --check-files`、`validate-plan --check-files` 闭环一起带出来，不能只停在 signal 文件生成
...
- `auto-cut-silence` 虽放在 `AudioCommandHandlers.cs`，但当前仍只是 `v1-compatible` 的显式辅助命令；若后续继续长出 `auto-cut-beats` / `auto-cut-transcript` 等命令，再评估是否单独抽 planning command owner
- `export` / `mix-audio` / `render` wrapper 已迁到 `RenderCommandHandlers.cs`；继续重构时，应优先在命令族文件内扩展，并把仅服务执行命令的 plan loading glue 保持在同一 owner 内
- 模板 guide / commands / preview 的组织逻辑已迁到 `TemplateCommandPresentation.cs`；继续重构时应优先复用这里，而不是再把相同的模板输出拼装塞回 `Program.cs`
- `scaffold-template-batch` 的 manifest contract 和工作目录约定必须保持收敛：第一版固定 `schemaVersion + items[] + id/input/template/workdir/validate/checkFiles`，未显式 `workdir` 时默认落到 `tasks/<id>`，根目录固定写 `summary.json`，每个条目固定写 `results/<id>.json`
...
- CLI 输出契约变化
- narrated-slides manifest 装载、失败 envelope 或探测 fallback 语义变化
- `init-narrated-plan --vars` 的装载约定或 overlay 语义变化
- narrated `bgm.slot` 的装载语义或错误路径变化
- narrated `visual.slot` 的装载语义、缺视觉探测回退或错误路径变化
