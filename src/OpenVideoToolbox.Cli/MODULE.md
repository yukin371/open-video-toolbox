...
- narrated-slides manifest 装载、相对路径解析、时长探测 glue 与 command envelope 输出的组织边界（当前 owner 文件：`NarratedSlidesPlanBuildSupport.cs`、`TemplateCommandHandlers.cs`）
...
- `init-narrated-plan` 只负责消费 narrated manifest、解析相对路径、在必要时通过 `ffprobe` 探测未显式声明的 section 时长，并把 narrated plan 生成规则留在 `Core.Editing`
