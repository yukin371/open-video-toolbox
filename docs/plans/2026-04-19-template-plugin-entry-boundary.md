...
- 当前仓库已经进入 `Hardening`，短期重点不是继续堆模板数量，而是先把模板扩展面的边界定清楚。
- `roadmap` 已明确“先模板插件，再能力插件”，但仓库内还没有一份可执行的入口草案。
- 如果不先定义发现、清单与 owner，后续很容易在 `templates`、`scaffold-template` 与外部模板仓库里各自发明一套元数据。

## 本轮目标

- 定义“模板插件优先”的最小扩展面。
- 明确插件发现、清单、目录约束和 CLI 责任。
- 保持 `Core` 仍是模板模型与 plan 语义的唯一 owner。
- 为未来实现留出 deterministic、可审计、可测试的入口。
...
1. 先把本草案挂进 `roadmap`，作为当前 Phase C 的边界基线。
2. 后续如果继续实现，只先做 `--plugin-dir` 的显式目录发现。
3. 为插件发现结果补 CLI 结构化输出测试。
4. 为插件模板解析复用现有 `Core.Editing` 测试与示例生成测试。
5. 在显式目录方案稳定前，不做用户级默认目录和远程分发。
