...
- 命令分发：`presets`、`templates`、`doctor`、`validate-plugin`、`init-plan`、`scaffold-template`、`scaffold-template-batch`、`beat-track`、`audio-analyze`、`audio-gain`、`audio-normalize`、`transcribe`、`detect-silence`、`separate-audio`、`probe`、`plan`、`run`、`cut`、`concat`、`extract-audio`、`subtitle`、`validate-plan`、`inspect-plan`、`replace-plan-material`、`attach-plan-material`、`bind-voice-track`、`bind-voice-track-batch`、`mix-audio`、`render`
- 参数解析、默认值选择、帮助输出
- CLI 级错误提示和退出码语义
- 稳定的结构化输出契约
...
- `scaffold-template-batch` 只负责解析 batch manifest、把相对路径解析到 manifest 目录、为每个条目复用单项 `scaffold-template` 执行路径，并汇总 `results[]` / `summary.json` / 稳定退出码；单任务 plan 生成、guide/example 写盘与 validation 语义仍由既有 owner 组合持有
...
- `scaffold-template-batch` 的 manifest contract 和工作目录约定必须保持收敛：第一版固定 `schemaVersion + items[] + id/input/template/workdir/validate/checkFiles`，未显式 `workdir` 时默认落到 `tasks/<id>`，根目录固定写 `summary.json`
