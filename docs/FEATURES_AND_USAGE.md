...
- 计划内素材工作流：`inspect-plan`、`replace-plan-material`、`replace-plan-material-batch`、`attach-plan-material`、`attach-plan-material-batch`、`bind-voice-track`、`bind-voice-track-batch`
...
| 计划内素材 | `replace-plan-material-batch` | 从 manifest 批量替换多份 plan 中已存在的素材绑定 | stdout / `summary.json` |
...
- 从 manifest 批量读取多份素材替换任务
- 逐项复用单项 replace 语义，不额外发明第二套 selector
- 会在 manifest 同目录固定写 `summary.json` 和 `results/<id>.json`

示例：

```powershell
<ovt> replace-plan-material-batch --manifest batch.json --json-out replace-plan-material-batch.json
```
