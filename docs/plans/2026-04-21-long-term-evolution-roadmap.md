...
| Core.Tests | 53 个 .cs，130 测试 |
| Cli.Tests | 59 个 .cs，118 测试 |
| CLI 命令 | 21 条 |
| CI | GitHub Actions: build + test + release workflow |
...
当前
  ...
  ├─ P1: 插件开发者体验 ─────────── (已完成)
  │
  ├─ D1: Desktop MVP ────────────── (候选阶段，待确认需求与低频变更窗口)
  │
  ├─ D2: Desktop 工作流收敛 ──────── (D1 完成后启动)
  │
  ├─ E1: 发布与分发 ────────────── (已完成，核心发布链已落地)
  │
  └─ E2: 生态与可持续演进 ────────── (下一候选阶段)
```
...
- `H1` 已完成，CLI hardening、统一 envelope、`doctor` 依赖语义与测试拆分停止线都已收口。
- `H2` 与 `T1` 已完成，契约快照测试、`--json-out` 补齐、ffmpeg CI 验证、模板 guide / scaffold / commands bundle 稳定化均已落地。
- `T2` 已完成，模板插件入口、`template.source` 审计链与插件 schema 校验已稳定。
- `P1` 已完成，插件开发指南、示例插件与插件验证错误路径测试均已落地。
- `E1` 的核心发布链已完成：tag 触发 release、跨平台 single-file 发布、版本策略与分层文档均已就绪。
- 包管理器渠道未在 `E1` 内落地；若后续仍有价值，转入 `E2` 统一评估。
- 当前不应再把 `H1` 视为活跃阶段；真正的下一决策点是：
  - 是否等待 `edit.json schema v1` 满足低频变更窗口后启动 `D1`
  - 或优先进入 `E2`，继续做生态、兼容性与分发渠道扩展
...
**候选阶段，待确认产品需求与 `edit.json schema v1` 低频变更窗口**
...
| 安装渠道 | 可选扩展：winget / Homebrew / NuGet global tool；若优先级不足，可后置到 `E2` |
