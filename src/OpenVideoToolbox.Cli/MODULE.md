...
- `bind-voice-track-batch` 负责解析批量 manifest、把相对路径解析到 manifest 目录、复用单项 `bind-voice-track` 流程，并返回结构化部分成功摘要与稳定退出码；单份 plan 的 mutation / validation 语义仍由 `Core.Editing` 持有
