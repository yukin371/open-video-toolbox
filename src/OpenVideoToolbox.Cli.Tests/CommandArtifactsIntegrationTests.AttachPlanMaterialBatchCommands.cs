...
{
    [Fact]
    public async Task AttachPlanMaterialBatch_WritesSummaryAndResultArtifacts()
    {
        var outputDirectory = Path.Combine(Path.GetTempPath(), $"ovt-attach-batch-{Guid.NewGuid():N}");
        Directory.CreateDirectory(Path.Combine(outputDirectory, "tasks", "job-a", "signals"));

        var planPath = Path.Combine(outputDirectory, "tasks", "job-a", "edit.json");
        var transcriptPath = Path.Combine(outputDirectory, "tasks", "job-a", "signals", "transcript.json");
        ...
        try
        {
            var result = await RunCliAsync("attach-plan-material-batch", "--manifest", manifestPath);
    ...
    [Fact]
    public async Task AttachPlanMaterialBatch_ReturnsPartialFailureSummary()
    {
        var outputDirectory = Path.Combine(Path.GetTempPath(), $"ovt-attach-batch-partial-{Guid.NewGuid():N}");
        Directory.CreateDirectory(Path.Combine(outputDirectory, "tasks", "job-a", "signals"));
            ...
            """
            {
              ...
              "items": [
                ...
                },
                {
                  ...
                  "path": "tasks/job-b/missing/transcript.json",
                  "transcript": true,
