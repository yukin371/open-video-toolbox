...
- 命令分发：`presets`、`templates`、`doctor`、`validate-plugin`、`init-plan`、`scaffold-template`、`scaffold-template-batch`、`beat-track`、`audio-analyze`、`audio-gain`、`audio-normalize`、`transcribe`、`detect-silence`、`separate-audio`、`probe`、`plan`、`run`、`cut`、`concat`、`extract-audio`、`subtitle`、`validate-plan`、`inspect-plan`、`replace-plan-material`、`replace-plan-material-batch`、`attach-plan-material`、`attach-plan-material-batch`、`bind-voice-track`、`bind-voice-track-batch`、`mix-audio`、`render`、`render-batch`
- 参数解析、默认值选择、帮助输出
- CLI 级错误提示和退出码语义
- 稳定的结构化输出契约
