...
- [ ] W1.1: `probe` 成功/失败都输出结构化 JSON，不退回 usage
- [ ] W1.2: `plan` 成功/失败都输出结构化 JSON，不退回 usage
- [ ] W1.3: `run` 成功/失败都输出结构化 JSON，不退回 usage
- [ ] W1.4: `doctor` 异常路径输出结构化 JSON，不退回 usage
- [ ] W1.5: `presets` 明确是否需要 envelope，结论写入文档
- [ ] W2.1: 所有 ffmpeg 命令都有 CLI 层真实 smoke
- [ ] W3: 测试拆分停止线有明确结论并写入 roadmap
- [ ] W4.1: 外部依赖安装指南已补齐
- [ ] W4.2: roadmap 阶段检查已更新
- [ ] `dotnet test OpenVideoToolbox.sln` 全绿
- [ ] 所有 21 条命令的成功路径都使用统一 command envelope
- [ ] 所有 21 条命令的失败路径都返回结构化 JSON，不退回纯 usage
- [ ] `dotnet test` 测试数量与改动前一致或更多
