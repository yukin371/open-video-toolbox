...
一个面向外部 AI 代理、脚本工作流与个人工作者的视频生产工具箱项目。

项目目标不是复刻旧式 GUI 工具，也不是把 AI 能力内嵌到软件里，而是提供一组可维护、可测试、可扩展的确定性命令，让 Claude、Codex 等外部工具能够组合这些 CLI 能力完成媒体处理、草稿生成、模板化剪辑编排与批量产出。

它不是“功能很少的轻工具”，而是优先整合个人工作者最常用、最可复用的视频生产能力：
...
- 面向个人工作者、独立创作者和自动化用户，而不是大型团队协作型 NLE
- 尽量覆盖大多数高频视频生产需求，而不是只做极窄单点工具
- 优先把高频能力整合成稳定 CLI 工作流，而不是做成重型专业编辑器
- 输出的重点是“可修改草稿”和“可批量复用模板化成品”
- 遇到更专业的调色、复杂时间线、深度特效和精修需求时，允许继续交给外部编辑器
